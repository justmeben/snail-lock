#!/bin/bash
# Snail Lock remote installer — no clone, no Xcode needed.
# Downloads the prebuilt app from the latest GitHub release and installs it:
#
#   curl -fsSL https://raw.githubusercontent.com/justmeben/snail-lock/main/remote-install.sh | bash
#
# Prompts for password + on-screen message, installs to /Applications, writes
# ~/.snail_lock.conf, and launches.

set -e

RELEASE_URL="https://github.com/justmeben/snail-lock/releases/latest/download/SnailLock.zip"
...
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
if ! curl -fsSL "$RELEASE_URL" -o "$TMP/SnailLock.zip"; then
    fail "Download failed: $RELEASE_URL (is there a published release with SnailLock.zip attached?)"
fi
ok "Downloaded"

(cd "$TMP" && unzip -q SnailLock.zip)
# Clean Apple's metadata sidecar so we don't accidentally select a hollow bundle.
rm -rf "$TMP/__MACOSX"
APP_SRC="$TMP/SnailLock/Snail Lock.app"
if [ ! -x "$APP_SRC/Contents/MacOS/snail-lock" ]; then
    fail "Bundle at $APP_SRC is missing its executable (corrupt download?)"
fi
ok "Unzipped"
